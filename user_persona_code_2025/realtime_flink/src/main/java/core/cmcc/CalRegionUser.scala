//package core
//
//import `case`.{RegionCount, RegionCell, UserInfo, UserRegionFlow, UserRegionInfo, XDR}
...
//    val xdrStream = env.addSource(kafkaConsumer).map(x=>XDR.fromKafka(x)).filter(x=> x!=null)
//    //xdrStream.print()
//
//    //获取用户基础数据-离线  9b0e2fa8-89e2-4b4c-9087-63afebe0847b|0|18
//    val userInfoFilePath = "I:\\data\\user_info.txt"  //"hdfs://192.168.182.100:9000/data/userInfo"
...
//    val regionCellFilePath = "I:\\data\\region_cell.txt" //"hdfs://bigdata01:9000/data/regionCell/region_cell.txt"
//    val regionCellStream = env.readFile(new TextInputFormat(new Path(regionCellFilePath)), regionCellFilePath, FileProcessingMode.PROCESS_CONTINUOUSLY,1000 * 2)
...
//
//
//    //******第二步：用户基站数据出入行政区、按用户做keyby-广播基站与区域关系数据，分析用户在每个行政区流入流出情况，保留用户上一个行政区状态******************
...
//    val userRegionFlowStream = laccellUserInfoStream
//      .keyBy(_.imsi)
//      .connect(broadcastRegionCellStream)
//      .process(new KeyedBroadcastProcessFunction[String, UserRegionInfo, RegionCell, UserRegionFlow] {
//
//          //存储flink中间状态
...
//
//          //处理基站信令(包括用户基础数据)数据
//          override def processElement(value: UserRegionInfo, ctx: KeyedBroadcastProcessFunction[String, UserRegionInfo, RegionCell, UserRegionFlow]#ReadOnlyContext, out: Collector[UserRegionFlow]): Unit = {
//            val imsi= value.imsi
//            val laccell = value.laccell
...
//            if (!region_id.equals(lastRegion.value()) && region_id.nonEmpty) {
//              out.collect(UserRegionFlow(imsi, 1, region_id, gender, age)) //1表示进入
//            }
//            if (lastRegion.value() != null) {
//                out.collect(UserRegionFlow(imsi, 0, lastRegion.value(), gender, age)) //流出上一个区域，也记录下来
//              }
//              lastRegion.update(region_id)  //更新用户的区域状态数据
//            }
//
//          //处理行政区-基站laccell关系数据，写入状态中
//          override def processBroadcastElement(value: RegionCell, ctx: KeyedBroadcastProcessFunction[String, UserRegionInfo, RegionCell, UserRegionFlow]#Context, out: Collector[UserRegionFlow]): Unit = {
//            //获取BroadcastState
//            val broadcastState = ctx.getBroadcastState(regionCellMapStateDescriptor)
